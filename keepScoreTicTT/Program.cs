...
namespace TicTacToe
{
    class Program
    {
        ...
        {
            bool win = false; //If the win is true, game is over
            while (!win)
            {
                string playerTurn = "X";
                int turn = 0;
                ...
                for (int i = 1; i <= 9; i++)
                {
                    printBoard(board); //Make the Board
                    Console.WriteLine("Player " + playerTurn);
                    Console.WriteLine("Enter Row:"); //player input
                    int row = Int32.Parse(Console.ReadLine());
                    Console.WriteLine("Enter Column:");//player input
                    ...
                    else//otherwise place a marker and change the turn
                    {
                        board[row, column] = playerTurn;
                        playerTurn = (playerTurn == "X") ? "O" : "X";
                    }

                    if ((2 % i) == 0) //increment turn every two moves
                    {
                        turn++;
                    }
