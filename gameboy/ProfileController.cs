...
namespace gameboy
{
    class ProfileController
    {
        ...
        public List<List<int>> scores { get; set; } = new List<List<int>>();

        public ProfileController()
        {
            for (int i = 0; i < game.Count; i++)
            {
                List<string> label = new List<string> { "Games Played", "wins", "Loss", "Times Cheated" };
                List<int> startScores = new List<int> { 0, 0, 0, 0 };
                labels.Add(label);
